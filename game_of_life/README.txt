...
width/height in ints, edge should be torus or hedge, defaults to hedge if otherwise. RGB settings included, s is the sprite size
(2, 4, 8, or 16), defaults to 8 otherwise. -f filename found in the conway_life folder, make sure to have the conway_life folder in the same directory/folder as all the other code, will seg fault otherwise. -o origin is an x,y point entered without spaces or else will seg fault.
